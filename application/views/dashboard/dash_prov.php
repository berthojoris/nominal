                                    ...
                                    <?php
                                        for($j=0;$j<count($prov[$i]);$j++){
                                            $total = $data_prov[$i][$j]['on']+$data_prov[$i][$j]['off']+$data_prov[$i][$j]['nop'];?>
                                                <a href="<?php echo base_url().'index.php/Dashboard/uker_prov/'.$prov[$i][$j]->kode_provider.'/'.$prov[$i][$j]->kode_jenis_jarkom.'/'.$prov[$i][$j]->brisat.'/'.$prov[$i][$j]->nickname_provider.'/'.$total;?>">
            ...
            for($i=0;$i<count($prov[$j]);$i++){
                ...
                var chartKanca = Highcharts.chart('container-<?php echo $prov[$j][$i]->id;?>', Highcharts.merge(gaugeOptions, {
                    yAxis: {
                        ...
                        max: 100,
                        title: {
                            text: '<div style="text-align:center"><span style="font-size:14px;font-weight: bold;"><?php echo $prov[$j][$i]->nickname_provider."<br>(".($data_prov[$j][$i]['on']+$data_prov[$j][$i]['off']+$data_prov[$j][$i]['nop']).")";?></span></div>'
                        }
                    },
