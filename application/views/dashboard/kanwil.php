...
<section class="content" style="margin-top:-30px;">   
    <div class="row">
        ...
        <div>
            <a href="<?php echo base_url();?>index.php/Dashboard/new_list_all"><button type="button" class="btn btn-primary btn-sm" style="width: 100px">List Remote</button></a>
        </div>
        <?php }?>
    </div>
</section>
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            for(var j=0 ; j<data["data"]["kode_kanwil"].length ;j++)
            {
                ...
                var warna='';
                var v_class='';
                if ((data['data']['persen'][j]['on']+data['data']['persen'][j]['off'])>0 && data['data']['persen'][j]['prosentase']<=80) {
                    warna = 'color:red';
                ...
                var prosen = data["data"]["persen"][j]["prosentase"].toFixed(2);
                 var chartKanca = Highcharts.chart('container-'+data["data"]["kode_kanwil"][j], Highcharts.merge(gaugeOptions, {
                    yAxis: {
                        ...
                        max: 100,
                        title: {
                            text: '<div style="text-align:center"><span style="font-size:18px;font-weight: bold;'+warna+'">'+data["data"]["nama_kanwil"][j].toUpperCase()+'<br>('+(parseInt(data["data"]['persen'][j]['all']))+')</span></div>'
                        }
                    },
                    ...
                    series: [{
                        ...
                        dataLabels: {
                            format: '<div style="text-align:center"><span '+v_class+' style="font-size:16px;color:' +
    ...
    var gaugeOptions = {
        ...
        <?php //foreach ($data as $datas){
            for($i=0;$i<count($data['kode_kanwil']);$i++){
                $warna='';
                $class='';
                if (($data['persen'][$i]['on']+$data['persen'][$i]['off'])>0 && $data['persen'][$i]['prosentase']<80) {
                    $warna = 'color:red';
                ...
                var chartKanca = Highcharts.chart('container-<?php echo $data['kode_kanwil'][$i];?>', Highcharts.merge(gaugeOptions, {
                    yAxis: {
                        ...
                        max: 100,
                        title: {
                            text: '<div style="text-align:center;"><span style="font-size:18px;font-weight: bold;<?php echo $warna;?>"><?php echo strtoupper($data['nama_kanwil'][$i])."<br>(".$data['persen'][$i]['all'].')';?></span></div>'
                        }
                    },
                    ...
                    series: [{
                        ...
                        dataLabels: {
                            format: '<div style="text-align:center"><span <?php echo $class;?> style="font-size:16px;color:' +
                                ((Highcharts.theme && Highcharts.theme.contrastTextColor) || 'black') + '">{y}</span><br/>' +
                                   '<span style="font-size:10px;color:silver"> % Online</span></div>'
                        },
                        tooltip: {
