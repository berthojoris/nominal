                        ...
                        <?php
                            for($j=0;$j<count($prov);$j++){
                                if($this->session->userdata('provider')==$prov[$j]->kode_provider){?>
                                    <a href="<?php echo base_url().'index.php/Dashboard/uker_prov/'.$prov[$i][$j]->kode_provider.'/'.$prov[$j]->kode_jenis_jarkom.'/'.$prov[$j]->brisat.'/'.$prov[$i][$j]->nickname_provider.'/';?>">
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            for(var j = 0 ; j < data["prov"].length ;j++)
            {
                ...
                var chartKanca = Highcharts.chart('container-'+data["prov"][j]["id"], Highcharts.merge(gaugeOptions, {
                    yAxis: {
                        min: 80,
                        max: 100,
                        title: {
    ...
    var gaugeOptions = {
        ...
        <?php
            ...
            for($j=0;$j<count($prov);$j++){
                ...
                ?>
                var chartKanca = Highcharts.chart('container-<?php echo $prov[$j]->id;?>', Highcharts.merge(gaugeOptions, {
                    yAxis: {
                        min: 80,
                        max: 100,
                        title: {
