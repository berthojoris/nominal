...
class Ticketremedy {
	...
	function postTicketRemedy($dynamicIP, $description, $notes, $proxyHost=null, $proxyPort=null, $proxyUsername=null, $proxyPassword=null) {
		...
		$client = new nusoap_client('http://web.brisolutions.bri.co.id/arsys/services/ARService?server=arapp1&webService=HPD_IncidentInterface_Create_WS', '', $proxyhost, $proxyport, $proxyusername, $proxypassword);
		$err = $client->getError();
		if ($err) {
    ...
    function getTicketRemedy($dynamicIP, $idAlarm, $proxyHost=null, $proxyPort=null, $proxyUsername=null, $proxyPassword=null) {
    	...
    	$client = new nusoap_client('http://web.brisolutions.bri.co.id/arsys/services/ARService?server=arapp1&webService=BRI:INC:GetInfoFromIPAddress', '', $proxyhost, $proxyport, $proxyusername, $proxypassword);
		$err = $client->getError();
		if ($err) {
			echo '<h2>Constructor error</h2><pre>' . $err . '</pre>';
		}
