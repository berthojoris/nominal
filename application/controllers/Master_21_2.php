...
class Master extends CI_Controller
{
	...
	function GetAlarmRemote()
	{
		...
		$columns = array("No","Site Id"/*,"Remote Name","Remote Type","Region","Main Branch","Branch Code","IP Address","Priority"*/,"Provider","Network Type","Current Type","Ack Time","Acked By","Alarm Start","Alarm End"); // data array columns harus sama dengan data header table yang tadi di buat di view
	...
	function ActionAckByRemote()
	{
		...
		if ($cek[0]->current_state == 1) {
			...
			$return = $this->db->set('status_alarm',3)->where('id_remote',$cek[0]->id_remote)->update('tb_remote');
			if ($return) {
				$return = 1;
			}else{
				$return = 0;
	...
	function ActionAckByJarkom()
	{
		...
		if ($cek[0]->current_state == 1) {
			...
			$return = $this->db->set('status_alarm',3)->where('id_jarkom',$cek[0]->id_jarkom)->update('tb_jarkom');
			if ($return) {
				$return = 1;
			}else{
				$return = 0;
    ...
    $data = array(
		...
		if($cek[0]->id_jarkom == null){
			$return = $this->db->set('notes_alarm',$note)->where('id',$cek[0]->id_jarkom)->update('tb_jarkom');
		}else{
			$return = $this->db->set('notes_alarm',$note)->where('id_remote',$cek[0]->id_remote)->update('tb_remote');
		}
