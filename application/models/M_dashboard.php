...
class M_dashboard extends CI_Model {
    ...
    {
    	$jum = $this->db->select('kode_jarkom')->from('tb_jarkom')->like('kode_jarkom','000','after')->order_by('kode_jarkom','desc')->limit(1)->get()->result();
	...
	function getjarkombykode($kode_jarkom)
	{

		$data = $this->db->select('*')
					->from('v_all_remote_jarkom')
					->where('kode_jarkom',$kode_jarkom)
					->where('used_status',1)
					->get()->result();
	...
	function getTid($id_remote)
	{

		$data = $this->db->select('tid_atm,name,name_head,telp,jenis')
					->from('v_atm')
					->where('id_remote',$id_remote)
					->get()->result();
	...
	function data_jarkom_disable()
    {
    	$data = $this->db->select('*')
							->from('v_all_remote_jarkom')
							->where('used_status','0')
							->order_by("id_remote", "asc")
							->get()->result();
    	// $data = $this->db->select('x.kode_jarkom as koja,x.kode_jarkom as kode_jarkom,a.*,b.status as status_onoff,c.tipe_uker,b.last_update as last_up,b.status_fail_date,b.status_rec_date,d.nama_kanca,e.nama_kanwil,f.keterangan as op')
					// 		->from('tb_jarkom x')
	...
	function getLocInduk($idremote,$tipe_uker)
	{
		...
		// $sql = "SELECT a.latitude, a.longitude
		// 		FROM tb_remote a
		// 		WHERE a.kode_kanca = ( SELECT b.kode_kanca FROM tb_remote b WHERE b.id_remote = ".$idremote." ) AND a.kode_tipe_uker=2;";
		// $query = $this->db->query($sql)->result();
		
		/*
		if (empty($query))
		{
			$query = array('latitude'=>'-6.216897','longitude'=>'106.814618');
		}else{
			$query = array('latitude'=>$query[0]->latitude,'longitude'=>$query[0]->longitude);
		}
		*/
		return $query;
	}
